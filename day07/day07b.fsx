...
let current_free_space = DirectorySize - total_size
let space_to_delete =  RequiredFreeSpace - current_free_space

let delete_candidate =
    tree_accumulated
    |> Map.filter (fun dir size -> size >= space_to_delete)
    |> Map.values
    |> Seq.min

printfn "Delete candidate size: %d" delete_candidate